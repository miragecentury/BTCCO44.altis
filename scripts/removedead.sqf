...
private["_viewDist","_manTimer","_vehTimer"];
// true/false = on/off.
...
_delete = {
    ...
    if (!isNull _del) then {
        _arr = [];
        waitUntil {sleep 10; ({isPlayer _x AND (_del distance (vehicle _x)) < _dis} count playableUnits) == 0};
        waitUntil {sleep 10; ({isPlayer _x AND (_del distance (vehicle _x)) < _dis} count switchableUnits) == 0};
