    ...
    while {_run} do 
    {
        ...
        if (_deserted > 0) then
        {
            if ((getPosASL _unit distance _position > 10) and ({alive _x} count crew _unit == 0) and (getDammage _unit < 0.8)) then 
            {
                ...
                waitUntil {sleep 0.5; (_timeout < time or !alive _unit or {alive _x} count crew _unit > 0)};
                if ({alive _x} count crew _unit > 0) then {_dead = false; _dead2 = false;}; 
                if ({alive _x} count crew _unit == 0) then {_dead = true; _nodelay =true; _dead2 = false;}; 
				if !(alive _unit) then {_dead = true; _nodelay = false; _dead2 = false;}; 
            };
        };
        ...
        if (_dead) then 
        {	
			//waitUntil{sleep 10; ((getPos Player) distance (_position) > 500)};  
			//By Muttley
			waitUntil 
				{sleep 2; 
					(
					(({(isPlayer _x) && (vehicle _x isKindOf "Man") && ((_x distance _position) < 1000)} count allUnits) < 1)&&
					(({(isPlayer _x) && (vehicle _x isKindOf "LandVehicle") && ((_x distance _position) < 1000)} count allUnits) < 1)&&
					(({(isPlayer _x) && (vehicle _x isKindOf "Air") && ((_x distance _position) < 1000)} count allUnits) < 1)
					)
				};
