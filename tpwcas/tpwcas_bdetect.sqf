...
bdetect_name 		= "bDetect | ArmA 2 Bullet Detection Framework";
...
bdetect_fnc_init =
{
	...
	if( isMultiPlayer && bdetect_mp_per_frame_emulation ) then {  // emulated, per-timeout (MP)
	...
	//if ( isServer ) then // EH's only needed at server or singleplayer system
	//{
...
bdetect_fnc_eh_add =
{
	...
	// Reset Fired EH after respawn - only needed for MP tpwcas_mode 2
	if ( _reset == 1 ) then
	{
...
bdetect_fnc_killed =
{
	...
	// remove fired EH
	[_unit, 1] spawn bdetect_fnc_eh_add;

	if ( bdetect_debug_enable ) then {
		_msg = format["bdetect_fnc_killed() - unit=%1, KILLED EH triggered - killed by [%2]", _unit, _killer];
		[ _msg, 3 ] call bdetect_fnc_debug;
	};
...
bdetect_fnc_find_other_ammo =
{
...

    {
		...
		} forEach ["GrenadeHandTimedWest", "GrenadeBase", "RocketBase", "MissileBase"];
		...
		else
		{
			if( _dist > bdetect_bullet_min_distance	) then
			{
...

				{
					if( _x != _shooter ) then
					{
						if( !(_x in _blacklist) && { (local _x) } && { (vehicle _x == _x) } && { (lifestate _x == "ALIVE") } ) then
						{
						...
						else
						{
							...
							if( bdetect_debug_enable ) then {
								_msg = format["bdetect_fnc_detect_bullet_sub() - blacklisted: unit=%1, shooter=%2, bullet=%3", _x, _shooter, _bullet];
...
bdetect_fnc_detect_projectile_sub =
{
	...
	for "_n" from 0 to _tot - 1 step 2 do
	{
		...
		if( !( isNull _projectile ) ) then
		{
			...
			//hint format ["NR: %4 - Pro: %1 - Life: %2 - Dist: %3", _projectile_max_proximity, _projectile_max_lifespan, _projectile_max_distance, _nr];
		...
		//|| ( ( _bpos select 2) > bdetect_projectile_max_height && ( ( vectordir _projectile ) select 2 ) > 0.2 )
		) then {
...

				{
					if( _x != _shooter ) then {
						...
						if( !(_x in _blacklist)  ) then {
							...
							if( (vehicle _x == _x) && { (local _x) } && { (lifestate _x == "ALIVE") } ) then {
