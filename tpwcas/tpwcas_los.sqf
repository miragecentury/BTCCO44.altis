...
tpwcas_los_fnc_mainloop =  
{
	...
	{ // start foreach
		...
		if ( (diag_ticktime >= _nexttime) && (lifestate _unit == "ALIVE") && !(side _unit == civilian) ) then  
		{	 
			_unit setvariable ["tpwcas_los_nexttime", diag_ticktime + random 1]; 
			...
			{ 
				...
				if ( !(isPlayer _near) && (((side _unit) getFriend (side _near)) < 0.6) && !(side _near == civilian) && (lifestate _near == "ALIVE") && ((_near knowsAbout _unit) < tpwcas_los_knowsabout) ) then
				{
